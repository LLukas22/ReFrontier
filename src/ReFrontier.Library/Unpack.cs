...
namespace ReFrontier.Library
{
    public class Unpack
    {
        ...
        public static string UnpackJPK(string input,string output_name)
        {
            ...
            if (br.ReadUInt32() == 0x1A524B4A)
            {
                ...
                Console.WriteLine($"JPK-File is of Type: {type}");
                switch (type)
                {
                    case 0:
                ...
                if (decoder != null)
                {
                    ...
                    FileInfo fileInfo = new FileInfo(input);
                    output = Path.Combine(fileInfo.DirectoryName, $"{output_name}.{extension}");
